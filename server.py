...
def _open_download(url: str, headers: dict | None = None):
    ...
    for u in urls:
        for i in range(4):
            try:
                req = urllib.request.Request(u, headers={"User-Agent": UA, **(headers or {})})
...
def civitai_download(url: str, dest: Path, dl_id: str, headers: dict | None = None) -> None:
    ...
    try:
        ...
        with DL_LOCK:
            DOWNLOADS[dl_id]["status"] = "error: " + dl_error_text(exc)
        raise
